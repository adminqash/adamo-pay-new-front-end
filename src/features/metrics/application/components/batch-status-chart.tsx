...
function BatchStatusTooltip({ active, payload, total, batchesLabel }: PieTooltipProps) {
...
export function BatchStatusChart({
  ...
  const data: BatchData[] = propData ?? [];
  const displayTotal = totalBatches || data.reduce((sum, item) => sum + item.value, 0);
          ...
          <PieChart>
            ...
            <Tooltip
              content={(
                <BatchStatusTooltip
                  total={displayTotal}
                  batchesLabel={t("metrics.batch_status.batches")}
                />
              )}
              cursor={false}

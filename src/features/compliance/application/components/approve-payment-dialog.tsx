...
type ApprovePaymentDialogProps = {
  open: boolean
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{t("compliance:review.approve_title")}</DialogTitle>
        </DialogHeader>
        <DialogBody className="flex flex-col gap-4">
          <Label className="text-sm text-neutrals-700">
            {t("compliance:review.approve_instruction")}
          </Label>
          <textarea
            value={note}
            onChange={(event) => setNote(event.target.value)}
            placeholder={t("compliance:review.approve_placeholder")}
